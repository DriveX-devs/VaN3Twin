...
namespace ns3
{
  NS_LOG_COMPONENT_DEFINE("CABasicServiceV1");

  CABasicServiceV1::~CABasicServiceV1() {
    NS_LOG_INFO("CABasicServiceV1 object destroyed.");
  }
  ...
  CABasicServiceV1::CABasicServiceV1(unsigned long fixed_stationid,long fixed_stationtype,VDP* vdp, bool real_time, bool is_vehicle)
  {
    m_socket_tx=NULL;
    m_btp = NULL;
  ...
  CABasicServiceV1::CABasicServiceV1(unsigned long fixed_stationid,long fixed_stationtype,VDP* vdp, bool real_time, bool is_vehicle, Ptr<Socket> socket_tx)
  {
    CABasicServiceV1(fixed_stationid,fixed_stationtype,vdp,real_time,is_vehicle);

    m_socket_tx=socket_tx;
  }

  void
  CABasicServiceV1::setStationProperties(unsigned long fixed_stationid,long fixed_stationtype)
  {
  ...
  void
  CABasicServiceV1::setFixedPositionRSU(double latitude_deg, double longitude_deg)
  {
    m_vehicle = false;
    m_RSUlon = longitude_deg;
    m_RSUlat = latitude_deg;
    //High frequency RSU container
    m_protectedCommunicationsZonesRSU = asn1cpp::makeSeq(RSUContainerHighFrequencyV1);
    auto protectedComm = asn1cpp::makeSeq(ProtectedCommunicationZoneV1);
  ...
  void
  CABasicServiceV1::setStationID(unsigned long fixed_stationid)
  {
    m_station_id=fixed_stationid;
  ...
  void
  CABasicServiceV1::setStationType(long fixed_stationtype)
  {
    m_stationtype=fixed_stationtype;
  ...
  void
  CABasicServiceV1::setSocketRx (Ptr<Socket> socket_rx)
  {
    m_btp->setSocketRx(socket_rx);
    m_btp->addCAMRxCallback (std::bind(&CABasicServiceV1::receiveCam,this,std::placeholders::_1,std::placeholders::_2));
  }

  void
  CABasicServiceV1::startCamDissemination()
  {
    if(m_vehicle)
      {
        m_event_camDisseminationStart = Simulator::Schedule (Seconds(0), &CABasicServiceV1::initDissemination, this);
      }
    else
      {
        m_event_camDisseminationStart = Simulator::Schedule (Seconds (0), &CABasicServiceV1::RSUDissemination, this);
      }
  }

  void
  CABasicServiceV1::startCamDissemination(double desync_s)
  {
    if(m_vehicle)
      {
        m_event_camDisseminationStart = Simulator::Schedule (Seconds (desync_s), &CABasicServiceV1::initDissemination, this);
      }
    else
      {
        m_event_camDisseminationStart = Simulator::Schedule (Seconds (desync_s), &CABasicServiceV1::RSUDissemination, this);
      }
  }

  void
  CABasicServiceV1::receiveCam (BTPDataIndication_t dataIndication, Address from)
  {
    Ptr<Packet> packet;
    asn1cpp::Seq<CAMV1> decoded_cam;
    ...
    /** Decoding **/
    decoded_cam = asn1cpp::uper::decodeASN(packetContent, CAMV1);
    ...
    if(m_LDM != NULL){
      //Update LDM
      vLDM_handler(decoded_cam, ll_addr);
    }
  ...
  void
  CABasicServiceV1::vLDM_handler(asn1cpp::Seq<CAMV1> decodedCAM, Address gn_addr)
  {
      vehicleData_t vehdata;
      LDM::LDM_error_t db_retval;
      bool lowFreq_ok;
      vehdata.gn_addr = gn_addr;
  ...
  void
  CABasicServiceV1::initDissemination()
  {
  ...
  void
  CABasicServiceV1::RSUDissemination()
  {
    generateAndEncodeCam();
    m_event_camRsuDissemination = Simulator::Schedule (MilliSeconds(m_RSU_GenCam_ms), &CABasicServiceV1::RSUDissemination, this);
  }

  void
  CABasicServiceV1::checkCamConditions()
  {
    int64_t now=computeTimestampUInt64 ()/NANO_TO_MILLI;
    CABasicServiceV1_error_t cam_error;
  ...
  CABasicServiceV1::generateAndEncodeCam()
  {
    VDP::CAM_mandatory_data_t cam_mandatory_data;
    CABasicServiceV1_error_t errval=CAMV1_NO_ERROR;
    ...
    auto cam = asn1cpp::makeSeq(CAMV1);

    if(bool(cam)==false)
      {
        return CAMV1_ALLOC_ERROR;
      }

    /* Fill the header */
    asn1cpp::setField(cam->header.messageID, FIX_CAMID);
    asn1cpp::setField(cam->header.protocolVersion , 1);
    ...
    */
    ...
    if(m_vehicle==true)
      {
        ...
        asn1cpp::setField(cam->cam.camParameters.highFrequencyContainer.present, HighFrequencyContainerV1_PR_basicVehicleContainerHighFrequency);
        ...
        if(swa.isAvailable()) {
            auto swa_seq = asn1cpp::makeSeq(SteeringWheelAngleV1);
        ...
        if(latacc.isAvailable()) {
            auto latacc_seq = asn1cpp::makeSeq(LateralAccelerationV1);
        ...
        if(vertacc.isAvailable()) {
            auto vertacc_seq = asn1cpp::makeSeq(VerticalAccelerationV1);
        ...
        if(tollZone.isAvailable ()) {
            auto tollZone_seq = asn1cpp::makeSeq(CenDsrcTollingZoneV1);
   ...
   else
      {
        ...
        asn1cpp::setField(cam->cam.camParameters.highFrequencyContainer.present,HighFrequencyContainerV1_PR_rsuContainerHighFrequency);
    ...
    if(m_lowFreqContainerEnabled == true)
    {
      ...
      if(lastCamGenLowFrequency==-1 || (computeTimestampUInt64 ()-lastCamGenLowFrequency)/1e6>=500.0)
      {
        ...
        auto lowFreqContainer = asn1cpp::makeSeq(LowFrequencyContainerV1);
        ...
        asn1cpp::setField(lowFreqContainer->present,LowFrequencyContainerV1_PR_basicVehicleContainerLowFrequency);
        ...
        if(!m_refPositions.empty ())
        {
          ...
          auto curr_ph = asn1cpp::makeSeq(PathPointV1);
          ...
          for(size_t i=1;i<m_refPositions.size ();i++)
            {
              curr_ph = asn1cpp::makeSeq(PathPointV1);
...
//     Special vehicle containers
    ...
    {
      auto specialVehicleCont = asn1cpp::makeSeq(SpecialVehicleContainerV1);
        ...
        case VehicleRole_publicTransport:
          {
            ...
            if(publicTransportContainerData.isAvailable ())
              {
                asn1cpp::setField(specialVehicleCont->present,SpecialVehicleContainerV1_PR_publicTransportContainer);
                ...
                if(publicTransportContainerData.getData ().ptActivationType.isAvailable ())
                  {
                    auto ptactivation_seq = asn1cpp::makeSeq(PtActivationV1);
            ...
            if(specialTransportContainerData.isAvailable ())
              {
                ...
                asn1cpp::setField(specialVehicleCont->present,SpecialVehicleContainerV1_PR_specialTransportContainer);
          ...
          if(dangerousGoodsBasic.isAvailable ())
            {
              ...
              asn1cpp::setField(specialVehicleCont->present,SpecialVehicleContainerV1_PR_dangerousGoodsContainer);
            ...
            if(roadWorksContainerBasicData.isAvailable ())
              {
                ...
                asn1cpp::setField(specialVehicleCont->present,SpecialVehicleContainerV1_PR_roadWorksContainerBasic);
                ...
                if((roadWorksContainerBasicData.getData ().drivingLaneStatus.isAvailable ()) ||
                  ...
                  {
                    auto closeLanes_seq = asn1cpp::makeSeq(ClosedLanesV1);
                    if(roadWorksContainerBasicData.getData ().drivingLaneStatus.isAvailable ())
                      {
            ...
            if(rescueContainerData.isAvailable ())
              {
                ...
                asn1cpp::setField(specialVehicleCont->present,SpecialVehicleContainerV1_PR_rescueContainer);
          ...
          if(emergencyContainerData.isAvailable ())
            {
              ...
              asn1cpp::setField(specialVehicleCont->present,SpecialVehicleContainerV1_PR_emergencyContainer);
              ...
              if(emergencyContainerData.getData().causeCode.isAvailable ())
                {
                  auto cause_seq = asn1cpp::makeSeq(CauseCodeV1);
            ...
            if(safetyCarContainerData.isAvailable ())
              {
                ...
                asn1cpp::setField(specialVehicleCont->present,SpecialVehicleContainerV1_PR_safetyCarContainer);
                ...
                if(safetyCarContainerData.getData ().incidentIndicationCauseCode.isAvailable ())
                  {
                    auto causeCode_seq = asn1cpp::makeSeq(CauseCodeV1);
    ...
    {
      return CAMV1_ASN1_UPER_ENC_ERROR;
    }
  ...
  uint64_t
  CABasicServiceV1::terminateDissemination()
  {
  ...
  int64_t
  CABasicServiceV1::computeTimestampUInt64()
  {
